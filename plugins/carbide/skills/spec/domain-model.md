...
The entities in the customer's shop and the relationships between them. This is a
...
- **Words the platform already owns.** MachineMetrics already defines machine, part, job, and
  operator. If the customer's meaning differs from the platform's, say so out loud and pick a
...
each answer from the customer, then hand all five to `storage-fit`.
...
can never be changed, and a model that does not fit needs the customer's own service instead.
